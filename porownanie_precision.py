...
names_column_1 = np.expand_dims(np.array(list(labels.keys())), axis=1)
scores_M = np.concatenate((names_column_1, mean_scores), axis=1)
scores_M = tabulate(scores_M, headers, tablefmt="2f")

t_statistic = np.zeros((len(preprocs), len(preprocs)))
p_value_1 = np.zeros((len(preprocs), len(preprocs)))
...
for label in range(len(labels)):
    ...
    for i in range(len(preprocs)):
        for j in range(len(preprocs)):
            t_statistic[i, j], p_value_1[i, j] = ttest_ind(scores_label[i], scores_label[j])

    advantage_1 = np.zeros((len(preprocs), len(preprocs)))
    advantage_1[t_statistic > 0] = 1
    advantage_1_table = tabulate(np.concatenate(
        (names_column, advantage_1), axis=1), headers)

    significance_1 = np.zeros((len(preprocs), len(preprocs)))
    significance_1[p_value_1 <= alfa] = 1
    significance_1_table = tabulate(np.concatenate(
        (names_column, significance_1), axis=1), headers)

    sign_better = significance_1 * advantage_1
